ARCH=x86
CCOPTIONS = -m32 -g -std=gnu99 -ffreestanding -O2 -Wall -Wextra -Wunused-parameter -Wint-to-pointer-cast -Wsign-compare -nostdlib -march=i486 -mtune=i486
ASPARAMS =
CCPATH = D:\os-gcc\bin
OBJECTS = arch\$(ARCH)\boot.o arch\$(ARCH)\terminal.o kernel\kernel.o arch\$(ARCH)\io.o arch\$(ARCH)\serial.o arch\$(ARCH)\arch.o arch\$(ARCH)\poweroff.o arch\$(ARCH)\gdt.o arch\$(ARCH)\gdtfl.o arch\x86\interrupts.o arch\x86\interruptsasm.o mm\mm.o mm\pmm.o mm\vmm_x86.o kernel\mstring.o kernel\process.o arch\$(ARCH)\switch.o arch\$(ARCH)\mmu.o kernel\syscall.o kernel\keyboard.o arch\x86\pic.o
%.o: %.c
